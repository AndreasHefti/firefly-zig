...
const start_scene_name = "scene1";
const end_scene_name = "scene2";

pub fn run(init_c: firefly.api.InitContext) !void {
    try firefly.init(init_c);
    defer firefly.deinit();

    firefly.Engine.start(screen_width, screen_height, 60, "Hello Room", init);
}
...
fn init() void {
    // we need to initialize the Tiled JSON integration tasks fist
    ...
    api.Task.runTaskByNameWith(
        game.Tasks.JSON_LOAD_TILED_ROOM,
        firefly.api.CallContext.new(
            null,
            .{
                .{ game.TaskAttributes.JSON_RESOURCE_ROOM_FILE, "resources/tiled/room1.json" },
